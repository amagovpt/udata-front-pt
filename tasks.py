...
def qa(ctx):
    ...
    with ctx.cd(ROOT):
        ...
        readme_results = ctx.run(
            'python setup.py check -m -s', pty=True, warn=True, hide=True)
...
def update(ctx):
    ...
    with ctx.cd(ROOT):
        ...
        for requirement_file in ["udata", "install", "test", "develop"]:
            ctx.run(
                f"pip-compile requirements/{requirement_file}.in "
                f"--output-file=requirements/{requirement_file}.pip --upgrade"
            )
