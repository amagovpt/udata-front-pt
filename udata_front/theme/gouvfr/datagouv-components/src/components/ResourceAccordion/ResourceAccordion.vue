    ...
    >
      <div>
        ...
        <div class="text-grey-380 subheaders-infos">
          ...
          <span class="fr-text--xs fr-mb-0 dash-after">{{
            t("Updated {date}", {
              date: formatRelativeIfRecentDate(lastUpdate),
            })
          }}</span>
          <span v-if="resource.format" class="fr-text--xs fr-mb-0 dash-after">
            ...
            <span v-if="resource.filesize"
              >({{ filesize(resource.filesize) }})</span
            >
          </span>
          <span
          ...
          >
            ...
            <span
              ...
              <span class="hidden show-on-small">{{
                t("downloads")
              }}</span></span
            >
          </span>
        ...
        >
          ...
          >
            <OrganizationNameWithCertificate
              :organization="communityResource.organization"
            />
          </a>
      ...
      <div class="flex items-center fr-ml-4v buttons">
        ...
        <p class="fr-col-auto fr-ml-3v fr-m-0" v-else>
          ...
          >
            ...
            <OhVueIcon
              ...
              class="fr-mr-2v"
            />
            <span class="fr-sr-only">{{ t("Download file as ") }}</span
            >{{ format }}
          </a>
        </p>
        ...
        <div
          ...
          :class="{
            'fr-icon-arrow-up-s-line': open,
            'fr-icon-arrow-down-s-line': !open,
          }"
        ></div>
      </div>
    </header>
          ...
          <TabPanel v-for="tab in tabsOptions" :key="tab.key">
            ...
            <div v-if="tab.key === 'downloads'" class="fr-pl-4v fr-pr-4v">
              <dl class="fr-pl-0">
                ...
                <dt class="font-bold fr-text--sm fr-mb-0" v-else>
                  {{ $t("Original format") }}
                </dt>
                <dd
                ...
                >
                  <span v-if="resource.format === 'url'">
                    ...
                    >
                      ...
                      >
                        <template #after>
                          <span
                            class="fr-ml-1v fr-icon-external-link-line fr-icon--sm"
                          ></span>
                        </template>
                      </TextClamp>
                    </a>
                  ...
                  <span v-else>
                    <span
                      class="text-blue-400 fr-icon-download-line fr-icon--sm fr-mr-1v fr-mt-1v"
                    ></span>
                    <a
                      :href="resource.latest"
                      class="fr-link"
                    ...
                    >
                      <span
                        >{{ $t("Format {format}", { format: resource.format })
                        }}<span v-if="resource.filesize">
                          - {{ filesize(resource.filesize) }}</span
                        ></span
                      >
                ...
                <template
                  v-if="resource.extras['analysis:parsing:parquet_url']"
                >
                  <dt class="font-bold fr-text--sm fr-mb-0">
                    {{
                      $t("Auto-generated formats from {platform}", {
                  ...
                  >
                    <span>
                      ...
                      >
                        <span
                          >{{ $t("Format {format}", { format: "parquet" })
                          }}<span
                            v-if="
            ...
            <div v-if="tab.key === 'swagger'" class="fr-pl-4v fr-pr-4v">
              <div>
                {{
                  t(
                    "Swagger automatically generated by data.gouv.fr. This swagger allows you to query data by API by filtering it by column value.",
                  )
                }}
              </div>
              <Swagger
                v-if="hasPreview"
                :url="`${config.tabular_api_url}/api/resources/${props.resource.id}/swagger/`"
              />
            </div>
          </TabPanel>
        </TabPanels>
...
const format = computed(() =>
  getResourceFormatIconSvg(props.resource) ? props.resource.format : t("File"),
);

const ogcService = computed(() =>
  OGC_SERVICES_FORMATS.includes(props.resource.format),
);
...
const availabilityChecked =
  props.resource.extras && "check:available" in props.resource.extras;
const unavailable =
  availabilityChecked && props.resource.extras["check:available"] === false;
const downloadButtonTitle = unavailable
  ? t(
      `The {certifier} robot failed to access this file - Download file as {format}`,
      { certifier: config.title, format: format.value },
    )
  : t(`Download file as {format}`, { format: format.value });

const resourceExternalUrl = computed(
  () =>
    `${window.location.origin}${window.location.pathname}#/${props.isCommunityResource ? "community-resources" : "resources"}/${props.resource.id}`,
);
